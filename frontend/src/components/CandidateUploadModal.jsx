      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl relative z-10 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)] overflow-hidden border border-slate-200 animate-in zoom-in-95 slide-in-from-bottom-10 duration-500">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-slate-900 tracking-tight">Add New Candidate</h2>
              <p className="text-slate-500 text-xs font-medium uppercase tracking-widest mt-0.5">Applicant Tracking System</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2.5 ml-1">
                  Full Name
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-900 font-medium focus:ring-4 focus:ring-teal-500/10 focus:border-teal-700 outline-none transition-all placeholder:text-slate-300"
                    value={formData.name}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-900 font-medium focus:ring-4 focus:ring-teal-500/10 focus:border-teal-700 outline-none transition-all appearance-none cursor-pointer"
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div 
                  ...
                  className="w-full px-4 py-4 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-slate-400 hover:border-teal-300 hover:bg-teal-50 transition-all cursor-pointer group"
                >
                  <Upload size={20} className="mb-2 group-hover:text-teal-600 transition-colors" />
                  {selectedFile ? (
                    <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                      <span className="text-xs font-medium text-teal-700 truncate max-w-[150px]">{selectedFile.name}</span>
                      <button 
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <span className="text-[10px] font-medium uppercase tracking-wider">Click to upload PDF</span>
                  )}
                </div>
              </div>
            </div>
