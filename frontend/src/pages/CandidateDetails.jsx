...
export default function CandidateDetails() {
  ...
  if (error || !candidate) {
    return (
      <div className="bg-rose-50 border border-rose-100 rounded-3xl p-12 text-center">
        ...
        <h2 className="text-2xl font-medium text-rose-900 mb-2">Candidate Not Found</h2>
        <p className="text-rose-700 mb-8">{error || "The candidate you are looking for does not exist or has been removed."}</p>
        <Link to="/candidates" className="inline-flex items-center gap-2 px-6 py-3 bg-white border border-rose-200 text-rose-700 font-medium rounded-xl hover:bg-rose-100 transition-all">
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <Link to="/candidates" className="inline-flex items-center gap-2 text-slate-500 hover:text-teal-700 font-medium text-sm transition-colors group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-10 flex flex-col md:flex-row gap-8 items-start">
                <div className="w-24 h-24 bg-teal-700 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-teal-100 flex-shrink-0">
                  <User size={40} />
                </div>
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    <h1 className="text-4xl font-semibold text-slate-900 tracking-tight">{candidate?.name || "Unknown Candidate"}</h1>
                    {(() => {
                      ...
                      return hasHiddenTalent && (
                        <span className="px-3 py-1 bg-emerald-500 text-white rounded-full text-xs font-semibold uppercase tracking-wider animate-pulse flex items-center gap-1 shadow-lg shadow-emerald-500/20 border border-emerald-400">
                          <Sparkles size={12} className="animate-spin-slow" /> Hidden Talent Mode Active
                        </span>
                      );
                    })()}
                  </div>
                  <p className="text-lg font-medium text-slate-500 mb-6">{candidate?.email || "No Email"}</p>
                  <div className="flex flex-wrap items-center gap-4 text-slate-500 mt-2">
                    <div className="flex items-center gap-1.5 text-sm font-medium px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl shadow-sm text-slate-700">
            ...
            <div className="p-10 pt-0 space-y-10">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                    <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Session ID</p>
                    <p className="text-sm font-mono text-slate-700">PH-{String(id).padStart(4, '0')}</p>
                  </div>
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                    <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Assessment Type</p>
                    <p className="text-sm font-medium text-slate-700">{candidate?.assessment_type || "General"}</p>
                  </div>
               </div>
               ...
               <div className="bg-white rounded-3xl p-10 border border-slate-200 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-3 mb-6 relative z-10">
                    ...
                    <h3 className="text-xl font-medium text-slate-900">AI Technical Evaluation</h3>
                  </div>
                  <p className="text-slate-600 leading-relaxed whitespace-pre-line relative z-10 text-base">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
               <div className="flex items-center gap-2">
                 ...
                 <h3 className="font-medium text-slate-900 tracking-tight">Security & Malpractice Report</h3>
               </div>
            ...
            <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
               <div className="space-y-6">
                  <div>
                    <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Malpractice Events</h4>
                    {(candidate?.has_malpractice) ? (
                      <div className="space-y-2">
                        {candidate.malpractice_flags.map((flag, idx) => (
                          <div key={idx} className="flex items-center gap-2 p-3 bg-rose-50 text-rose-700 text-xs font-medium rounded-xl border border-rose-100 shadow-sm">
                             <AlertTriangle size={14} /> {flag}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 p-3 bg-emerald-50 text-emerald-700 text-xs font-medium rounded-xl border border-emerald-100 shadow-sm">
               ...
               <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="font-medium text-slate-900">Authenticity Summary</h4>
                  </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
               <div className="flex items-center gap-2">
                 ...
                 <div>
                   <h3 className="font-medium text-slate-900 tracking-tight">Resume Claims vs. Coding Proof</h3>
                 ...
                 <div className="relative w-10 h-10 flex items-center justify-center">
                   <svg className="w-full h-full transform -rotate-90">
                     ...
                     <circle 
                       ...
                       cy="20" 
                       r="17" 
                       className="stroke-teal-700 fill-none transition-all duration-1000 ease-out" 
                       strokeWidth="4"
                   ...
                   <span className="absolute text-[10px] font-semibold text-slate-800">{candidate?.skill_authenticity_score || 0}%</span>
                 </div>
                 <div>
                   <p className="text-[9px] font-medium text-slate-500 uppercase tracking-widest leading-none mb-0.5">MATCH RATE</p>
            ...
            <div className="p-10 grid grid-cols-1 xl:grid-cols-3 gap-10">
               <div className="xl:col-span-2 space-y-4">
                  <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Extracted Skill Matrix</h4>
                  <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                          <th className="p-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Skill</th>
                          <th className="p-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Resume Claim</th>
                          <th className="p-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Proven Reality</th>
                          <th className="p-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Verification</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {Object.entries(candidate?.resume_skills || {}).map(([skill, claimedLvl]) => {
                          ...
                          if (claimedVal === provenVal) {
                            statusBadge = (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-teal-50 text-teal-800 rounded-lg text-xs font-medium border border-teal-100">
                                <CheckCircle2 size={12} /> Verified Match
                              </span>
                            );
                          } else if (claimedVal > provenVal) {
                            statusBadge = (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-700 rounded-lg text-xs font-medium border border-amber-100">
                                <AlertCircle size={12} /> Overclaimed
                              </span>
                            );
                          } else {
                            statusBadge = (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-medium border border-emerald-200 shadow-sm shadow-emerald-100/50 animate-pulse">
                          ...
                          return (
                            <tr key={skill} className="hover:bg-slate-50/50 transition-colors">
                              <td className="p-4 text-sm font-medium text-slate-900">{skill}</td>
                              <td className="p-4 text-sm font-medium text-slate-500">{claimedLvl}</td>
                              <td className="p-4 text-sm font-medium text-teal-700">{provenLvl}</td>
                        ...
                        {Object.keys(candidate?.resume_skills || {}).length === 0 && (
                          <tr>
                            <td colSpan="4" className="p-8 text-center text-sm text-slate-500 italic">No skills uploaded by candidate yet.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
               ...
               <div className="xl:col-span-1 bg-slate-50 rounded-3xl p-6 border border-slate-100 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="font-medium text-slate-900">Nudges & Guidance</h4>
                    </div>
                    {candidate?.growth_nudges?.length > 0 ? (
                      ...
                      <p className="text-xs text-slate-500 italic text-center py-8">No growth suggestions needed. Perfect score alignment.</p>
                    )}
                  </div>
               </div>
            </div>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-10 text-center space-y-8">
             <h3 className="text-xl font-medium text-slate-900 tracking-tight">Final Assessment</h3>
             ...
             <div className="pt-6 border-t border-slate-100 space-y-4">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Recruiter Decision</p>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-10 text-center space-y-6">
              <div className="bg-teal-50 w-16 h-16 rounded-2xl flex items-center justify-center text-teal-700 mx-auto">
                 <MessageSquare size={32} />
              </div>
              <div>
                <h4 className="text-lg font-medium text-slate-900 uppercase tracking-tight mb-2">Communication</h4>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <Link 
                    ...
                    className="block w-full py-4 bg-teal-700 text-white font-medium rounded-2xl hover:bg-teal-800 transition-all shadow-lg shadow-teal-100"
                  >
                    Go to Chat
                  </Link>
                  <button 
                    onClick={() => setIsScheduleModalOpen(true)}
                    className="block w-full py-4 bg-white border-2 border-slate-100 text-slate-700 font-medium rounded-2xl hover:bg-slate-50 transition-all"
                  >
                    Schedule Interview
                  </button>
                </div>
